...
def get_settings(address: Optional[str] = Query(None)):
    ...
    return {
        ...
        "sodex_api_key": config.get("sodex_api_key", ""),
        "account_id_mainnet": config.get("account_id_mainnet", ""),
        "network_mode": config.get("network_mode", "testnet"),
        "volume_farming": config.get("volume_farming", 0)
    }
...
async def save_settings(req: Request):
    ...
    sodex_api_key = data.get("sodex_api_key")
    account_id_mainnet = data.get("account_id_mainnet")
    network_mode = data.get("network_mode", "testnet")
    
    volume_farming = data.get("volume_farming", 0)
    ...
    if db:
        db.save_config(
            ...
            trading_mode,
            sodex_api_key,
            pk, # userPrivateKey is used for both testnet and mainnet (secret)
            account_id_mainnet,
            network_mode,
            volume_farming
        )
...
async def trade_unified(req: UnifiedTradeRequest):
    try:
        ...
        lev_res = await retry_async(temp_client.execute_leverage, lev_params, sig_lev, nonce_lev)
        if lev_res.get("code") != 0:
            return {"code": -1, "error": f"Leverage Sync Failed: {lev_res.get('error') or lev_res.get('msg')}"}

        # Step 2: Cleanup existing orders for this symbol first
        print(f">>> UNIFIED: Cleaning up existing orders for {req.symbol}...")
        try:
            # Create a temporary client with the user's private key for cleanup
            temp_client_cleanup = SodexClient(api_key_name=api_key, private_key=signing_key, network_mode=network_mode)
            await asyncio.to_thread(temp_client_cleanup.cancel_all_orders_for_symbol, int(req.account_id), req.symbol)
        except Exception as ce:
            print(f"DEBUG: Unified Cleanup Warning: {ce}")
...
async def close_position(req: CloseRequest):
    ...
    try:
        ...
        network_mode = user_conf.get("network_mode", "testnet")
        account_id = user_conf.get("account_id_mainnet") if network_mode == "mainnet" else user_conf.get("account_id")
...
async def check_account(address: str = Query(...)):
    try:
        ...
        if state and state.get("code") == 0:
            ...
            # Extract balance from 'B' array directly
            b_arr = data.get("B", [])
            if b_arr and isinstance(b_arr, list):
                for b_item in b_arr:
                    if b_item.get("a") == "vUSDC":
                        balance = b_item.get("wb", "0.00")
                        break
                else:
                    if len(b_arr) > 0:
                        balance = b_arr[0].get("wb", "0.00")
